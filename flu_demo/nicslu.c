...
int main(int argc[], char *argv[])
{
   ...
   // cfg[3] = 4;
    ...
    printf("Estimated number of nonzeros in L + U - I = %d\n", (int)stat[4]);
    printf("Runtime of factorization = %lf\n", stat[1]);
    printf("Number of off-diagonal pivots = %d\n", (int)stat[11]);
   ...
   // SparseResidual(n, ax, ai, ap, b, x, res, MATRIX_ROW_REAL);
    ...
    printf("sn has least %lf columns\n", cfg[8]);
...
//       printf("re-factor [%d] time: %g\n", j + 1, t2);
   ...
   /* for ( i = 0; i < n; i++ )
    ...
    printf("nnz(factors): %d\n", (int)stat[8]); //# of factors
    printf("nnz of LU = %d\n", (int)stat[9]+(int)stat[10]);
