  ...
  #pragma omp parallel
{
	...
	for ( kk = sum_level; kk < max_level; kk++ )
	  ...
	  {
		  {
			...
			//   k = kk;
			  ...
			  column_end_2 = row_ptr_U[offset_U[k] + wait_col - 1];
...
/*			  for ( j = 0; j < wait_col; j+=pack_j )
			  {
				  ...
				  if ( column_end_2-column_start > sn_record[column_start] ) column_number_sn = sn_record[column_start]+1;
				  else column_number_sn = column_end_2-column_start+1;
