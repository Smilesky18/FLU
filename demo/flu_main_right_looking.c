...
int main( int argc[], char *argv[])
{
 ...
 for(int i=0; i<num_thread; i++){
	...
	// int *row_ptr_L_plus = (int *)malloc(sizeof(int) * lnz+1);
	int *offset_L_plus = (int *)malloc(sizeof(int) * n+2);
	offset_L_plus[0] = 0;

	for ( i = 1; i < n+2; i++ )
	{
		offset_L_plus[i] = offset_L[i-1];
	}

	int *len_U = (int *)malloc(sizeof(int) * unz);
	int *lp_trans = (int *)malloc(sizeof(int) * n);
	memset(lp_trans, 0, sizeof(int) * n);
	for ( i = 0; i < n; i++ ) 
	{
		for ( j = offset_U[i]; j < offset_U[i+1]; j++ )
		{
			len_U[j] = j - offset_U[i];
			lp_trans[row_ptr_U[j]]++;
		}
	}
	int *offset_U_trans = (int *)malloc(sizeof(int) * n+1);
	int *row_ptr_U_trans = (int *)malloc(sizeof(int) * unz);
	offset_U_trans[0] = 0;
	for ( i = 1; i < n+1; i++ )
	{
		offset_U_trans[i] = offset_U_trans[i-1] + lp_trans[i-1];
		// printf("*******%d\n", lp_trans[i-1]);
	}
	int *depend_L_col = (int *)malloc(sizeof(int) * unz);
	int *len_U_in_computation = (int *)malloc(sizeof(int) * unz);
	for ( i = 0; i < n; i++ ) 
	{
		for ( j = offset_U[i]; j < offset_U[i+1]; j++ )
		{
			row_ptr_U_trans[offset_U_trans[row_ptr_U[j]]] = i;
			if ( row_ptr_U[j] == i ) depend_L_col[offset_U_trans[row_ptr_U[j]]] = -1;
			else depend_L_col[offset_U_trans[row_ptr_U[j]]] = row_ptr_U[j];
			len_U_in_computation[offset_U_trans[row_ptr_U[j]]] = len_U[j];
			offset_U_trans[row_ptr_U[j]]++;
		}
	}
	int k;
	double U_diag;
	...
	{
		k = row_perm_inv[i];
		for ( j = ap[k]; j < ap[k+1]; j++ )
		{
			xx[col_perm[ai[j]]] = ax[j];
		}
		for ( j = offset_U[i]; j < offset_U[i+1]; j++ )
		{
			U[j] = xx[row_ptr_U[j]];
	...
	for ( int jj = 0; jj < loop; jj++ )
	{
		...
		flu_right_looking(ax, ai, ap, n, lnz, unz, row_perm_inv, col_perm, row_ptr_L, offset_L, row_ptr_U, offset_U, sn_record, thresold, L, U, xx1, xx2, dv1, dv2, tag, num_thread, row_ptr_U_trans, depend_L_col, len_U_in_computation, offset_L_plus, offset_U_trans);
