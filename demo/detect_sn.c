...
/*	for ( i = 0; i <= sn_sum; i++ )
	{
		if ( sn_end[i] - sn_start[i] >= col_thresold )
		{
			sn_col_num = sn_end[i] - sn_start[i] + 1;
			sn_num = ( offset_L[sn_end[i]+1] - offset_L[sn_end[i]] - 1 ) * sn_col_num + (sn_col_num*(sn_col_num+1)) / 2 - sn_col_num;

			int divid = sn_num / 12000;
			...
			else
			{
				...
				for ( j = sn_start[i]; j <= sn_end[i]; j++ )
				{
					sn_s += offset_L[j+1] - offset_L[j] - 1;
					if ( sn_s >= 12000 )
					{
						for ( k = j_start; k <= j; k++ )
						{
		...
		// // if ( sn_num > 4000 ) 
		// if ( sn_end[i] - sn_start[i] >= col_thresold )	printf("sn %d: %d numbers %d cols\n", i, sn_num, sn_col_num);	
	}
*/
