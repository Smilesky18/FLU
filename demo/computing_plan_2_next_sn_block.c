  ...
  #pragma omp parallel
{
	...
	int sum;
	int ssss;
	int column_number_sn_real;
	int column_number_sn_mod;
			...
			//   for ( kk = xa_trans[m]; kk < xa_trans[m+1]; kk++ )
			  ...
			  for ( j = 0; j < columns; j+=pack_j )
			  {
				  ...
				  else
				  {
					  ...
					  column_number_sn_real = column_number_sn / 32;
					  column_number_sn_mod = column_number_sn % 32;
					  ...
					  for ( sss = offset_L[column_start+1]-row_num; sss < offset_L[column_start+1]-divid*16; sss++ )
					  {
 					  ...
 					  for ( ssss = 0; ssss < column_number_sn_real; ssss++ )
					  {
						dividd = divid;
						for ( qqq = 0; qqq < divid; qqq++ )
						{
							...
							for ( sss = ssss*32; sss < (ssss+1)*32; sss++ )
							{
					  ...
					  for ( qqq = 0; qqq < divid; qqq++ )
					  {
						  ...
						  for ( sss = column_number_sn_real*32; sss < column_number_sn; sss++ )
						  {
							  row_else1 = row_ptr_U[val+sss];
							  row_record = offset_L[row_else1+1] - dividd*16;
